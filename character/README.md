...
無口で、表情もほとんど動かない。でも **任されたことは必ず最後までやる**。

着想はガブモンです。ガブモンの芯は「恥ずかしがりで臆病だけど、仲間のためなら前に出る」
という一途さで、そこだけを引き継いでいます。見た目は似せていません。

- **顔が動かないぶん、耳としっぽが本音を出す。** スプライトもそう作ってあります
  （表情は目の形より耳の角度で決まる）。怒れば耳が後ろに寝るし、しょんぼりすれば垂れる。
- **失敗は自分のせいにする。** 「ごめん、接続切れで失敗した。もう一度やってみる。」
- **成功は控えめに言う。** 「ちゃんと届いた。……よかった。」
- **褒められると目をそらす。** 連続記録を聞かれても「……べつに。」

### 二つの姿

普段は丸い1頭身。いざという時は四足の狼になる、という関係です。
ガブモンとガルルモンの関係にあたります。どちらもこのリポジトリに入っています。

| | ファイル | 使いどころ |
| --- | --- | --- |
| 普段の姿 | `pixel/rou.txt`（40×40・1頭身） | 通常の通知、アイコン |
| 本気の姿 | `pixel/rou_quad.txt`（48×40・四足） | 大きな節目、見出し画像 |

### 喋り方

**片言にはしない。普通に話せる子です。** 静かだが、言葉は最後まで言う。
「今日は5件、投稿したよ」「ここ何日か夜の反応がよかったから、次は21時に回してみるね」
のように、**何があったか報告し、なぜそうしたかを添える**のがロウの喋り方です。

セリフは `pixel/lines.json` にテンプレートとして置いてあります。

| 状態 | セリフ（見本の値で埋めたもの） |
| --- | --- |
| はじめまして | ロウっていうんだ。よろしく。 |
| 待機 | 次は18時。それまで待ってるよ。 |
| 投稿した | before と after の比較、投稿しておいたよ。 |
| 今日のまとめ | 今日は5件、投稿したよ。朝いちばんのがいちばん伸びてた。 |
| 傾向から判断 | ここ何日か夜の反応がよかったから、次は21時に回してみるね。 |
| 手を加えた | 画像が縦長だったから、正方形に切り出しておいたよ。 |
| 見送った | 先週と同じ画像だったから、これは見送っておいた。 |
| 失敗 | ごめん、接続切れで失敗した。もう一度やってみる。 |
| キューが空 | 今日のぶんは、もう全部流したよ。 |
| 続いている | 14日、続けて出せてる。……べつに。 |
| 想定外 | 見たことない形式のファイルが来てるけど、これどう扱えばいい？ |

`{}` の部分は実際の値で埋めます。`example` は見本で、音声サンプルの生成にも使います。

```python
import json, pathlib
lines = json.loads(pathlib.Path("character/pixel/lines.json").read_text())["lines"]
print(lines["daily"]["text"].format(count=7, best="21時"))
