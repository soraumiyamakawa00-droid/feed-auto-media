...
![表情9種](pixel/sd_expressions_x6.png)

頭がそのまま体。手足は付けず、耳と丸いしっぽだけが生えています。

| ファイル | 表情 | フィードでの使いどころ |
| --- | --- | --- |
| `pixel/sprite_wolf_sd.txt` | 通常（既定） | 待機中 |
| `pixel/sprite_wolf_sd_bark.txt` | 吠える | 投稿した瞬間 |
| `pixel/sprite_wolf_sd_happy.txt` | 喜び | 投稿成功 |
| `pixel/sprite_wolf_sd_angry.txt` | 怒り | エラー・失敗 |
| `pixel/sprite_wolf_sd_sad.txt` | しょんぼり | 投稿スキップ |
| `pixel/sprite_wolf_sd_sleep.txt` | 寝ている | キューが空 |
| `pixel/sprite_wolf_sd_wink.txt` | ウインク | 予約完了 |
| `pixel/sprite_wolf_sd_smug.txt` | どや顔 | 連続投稿の達成 |
| `pixel/sprite_wolf_sd_surprise.txt` | 驚き | 想定外の入力 |

### 丸く見せる

丸く見せるのに効いたのは、輪郭を丸く描くことではなく
...
耳の角度は回転で作っています（立てる 0° ／ 少し寝かせる -25°（寝ている）／
後ろに寝かせる -45°（怒り）／垂らす -72°（しょんぼり））。
手で置くと頭の大きさを変えたときに耳が浮くので、必ず回転で作ります。

まぶたの帯はどの表情でも残し、**傾きだけ**変えています。
例外は驚きで、ここだけ帯を外します。帯を残すと見開いた感じが出ません。
